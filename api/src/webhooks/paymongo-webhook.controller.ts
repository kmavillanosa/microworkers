...
export class PaymongoWebhookController {
  ...
  async handlePaymongo(@Body() payload: PaymongoEventPayload) {
    ...
    if (eventType === 'payment.paid') {
      const paymentData = payload?.data?.attributes?.data as
        | PaymongoPaymentData
        | undefined;
      const paymentIntentId = paymentData?.attributes?.payment_intent_id;
      if (!paymentIntentId) return { received: true };
      const paymentIntent =
        await this.paymongoService.getPaymentIntent(paymentIntentId);
      let orderIdResolved =
        paymentIntent?.attributes?.metadata?.order_id?.trim() ?? null;
      if (!orderIdResolved) {
        const existingOrder =
          await this.ordersService.findOrderByPaymentSessionId(paymentIntentId);
        if (existingOrder?.id) orderIdResolved = existingOrder.id;
      }
      if (!orderIdResolved) {
        const pending =
          await this.ordersService.findPendingByCheckoutSessionId(
            paymentIntentId,
          );
        if (pending && typeof pending === 'object') {
          try {
            const dto = pending as unknown as CreateOrderDto;
            const created = await this.ordersService.create(
              dto,
              paymentIntentId,
            );
            orderIdResolved = created.id;
          } catch {
            // keep webhook acknowledged so PayMongo doesn't retry indefinitely
          }
        }
      }
      if (!orderIdResolved) return { received: true };
      ...
      try {
        ...
        await this.ordersService.deletePendingCheckout(paymentIntentId);
      } catch {
        //
      }
    ...
    const orderIdFromPayload =
      checkoutSessionPayload?.attributes?.metadata?.order_id;
    ...
    if (!orderIdResolved && sessionId) {
      const existingOrder =
        await this.ordersService.findOrderByPaymentSessionId(sessionId);
      if (existingOrder?.id) orderIdResolved = existingOrder.id;
    }
    ...
    if (!orderIdResolved && sessionId) {
      const payload =
        await this.ordersService.findPendingByCheckoutSessionId(sessionId);
