...
export class SlackService {
  ...
  private readonly webhookUrl =
    process.env.SLACK_ORDER_WEBHOOK_URL?.trim() ?? '';
  ...
  async notifyOrder(payload: SlackOrderPayload): Promise<void> {
    ...
    const who = customerName !== '—' ? customerName : customerEmail;
    const text = [
    ...
    try {
      this.logger.log(
        `Slack: sending order notification for order ${payload.orderId}`,
      );
      ...
      if (res.ok) {
        this.logger.log(
          `Slack: notification sent for order ${payload.orderId}`,
        );
      } else {
        this.logger.warn(
